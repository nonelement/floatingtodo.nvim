-- floating todo plugin
-- Floating TODO List Tutorial - Coding with Sphere; https://www.youtube.com/watch?v=7Kzv3wUHtyU
...
local function open_floating_file(target_file)
  ...
  if buf ==  -1 then
    buf = nvim.api.nvim_create_buf(false, false)
    vim.api.nvim_nbuf_set_name(buf, expanded_path)
  end
  ...
  vim.api.nvim_buf_set_keymap(buf, "n", "q", "", {
    ...
    callback = function ()
      if vim.api.nvim_get_option_value("modified", { buf = buf }) then
        vim.notify("save changes?", vim.log.levels.WARM)
      else
